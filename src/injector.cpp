...
bool EnableDebugPrivilege()
{
    ...
    if (!LookupPrivilegeValueW(nullptr, L"SeDebugPrivilege", &tp.Privileges[0].Luid)) {
        printf("Injector: LookupPrivilegeValue(SeDebugPrivilege) failed (error %d)\n", GetLastError());
        CloseHandle(hToken);
        return false;
    }
    ...
    if (!ok || err != ERROR_SUCCESS) {
        printf("Injector: AdjustTokenPrivileges(SeDebugPrivilege) failed (error %d)\n", err);
        return false;
    }
...
//   rcx = RemoteParam* ; returns HMODULE (or 0) and stores GetLastError in error_out
...
void LogRemoteError(DWORD code)
{
    ...
    switch (code)
    {
    case ERROR_FILE_NOT_FOUND:  name = L"ERROR_FILE_NOT_FOUND"; break;
    case ERROR_PATH_NOT_FOUND:  name = L"ERROR_PATH_NOT_FOUND"; break;
    case ERROR_ACCESS_DENIED:   name = L"ERROR_ACCESS_DENIED"; break;
    case ERROR_NOT_ENOUGH_MEMORY: name = L"ERROR_NOT_ENOUGH_MEMORY"; break;
    case ERROR_MOD_NOT_FOUND:   name = L"ERROR_MOD_NOT_FOUND (missing dependency?)"; break;
    case ERROR_PROC_NOT_FOUND:  name = L"ERROR_PROC_NOT_FOUND"; break;
    case ERROR_BAD_EXE_FORMAT:  name = L"ERROR_BAD_EXE_FORMAT (wrong architecture?)"; break;
    case ERROR_NOACCESS:        name = L"ERROR_NOACCESS"; break;
    }
...
bool InjectDLL(DWORD pid, const wchar_t* dll_path)
{
    ...
    LPVOID remote_base = VirtualAllocEx(hProcess, nullptr, total_size,
    ...
    if (!WriteProcessMemory(hProcess, remote_path, dll_path, path_size, nullptr)) {
    ...
    RemoteParam param = {};
    param.fnLoadLibraryW = GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "LoadLibraryW");
    param.fnGetLastError = GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "GetLastError");
    ...
    if (!WriteProcessMemory(hProcess, remote_param, &param, sizeof(param), nullptr)) {
    ...
    if (!WriteProcessMemory(hProcess, remote_stub, g_Stub, sizeof(g_Stub), nullptr)) {
    ...
    HANDLE hThread = CreateRemoteThread(hProcess, nullptr, 0,
        reinterpret_cast<LPTHREAD_START_ROUTINE>(remote_stub), remote_param, 0, nullptr);
    if (!hThread) {
    ...
    ReadProcessMemory(hProcess, &reinterpret_cast<RemoteParam*>(remote_param)->error_out,
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    if (!IsElevated()) {
        printf("Injector: NOT elevated - run from an elevated (Administrator) console\n");
